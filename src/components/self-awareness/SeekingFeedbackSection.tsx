...
export function SeekingFeedbackSection({ headingId }: Props) {
  ...
  const savedMessage = saved ? "Saved to your profile" : "";
  ...
  return (
    <div className="mx-auto max-w-[80rem] px-5 pb-16 pt-8 sm:max-w-[80rem] sm:px-10 sm:pb-24 sm:pt-12">
      <section
      ...
      >
        <div className="rounded-[2rem] bg-slate-50/80 p-6 shadow-[0_20px_60px_rgba(15,23,42,0.08)]">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-2 max-w-xl text-[0.95rem] leading-7 text-slate-600">
                  Specificity is the bridge to meaningful growth. Use these prompts to ask for clarity.
                </p>
              </div>
            </div>
          ...
          <div className="mt-6 rounded-[1.75rem] bg-white p-6 shadow-[0_12px_30px_rgba(15,23,42,0.08)]">
            <div className="space-y-4 text-slate-600">
              ...
              <ul className="space-y-3">
                {QUESTIONS.slice(0, 4).map((q) => (
                  <li key={q.title} className="flex items-start gap-3 text-[0.95rem] text-slate-700">
                    <span className="mt-1 block h-8 w-8 shrink-0 rounded-2xl bg-slate-100 text-center leading-8 text-slate-500">💬</span>
                    <span>{q.title}</span>
                  </li>
                ))}
              </ul>
                ...
                >
                  {QUESTIONS.map((question, index) => (
                    ...
                    >
                      ...
                      <h3 className="text-[1.25rem] sm:text-[1.35rem] break-words whitespace-normal font-semibold leading-8 text-slate-900">
                        {question.title}
                      </h3>
                      ...
                      <textarea
                        ...
                        className="mt-5 min-h-[15rem] w-full resize-y rounded-[1.5rem] border border-slate-200 bg-slate-50 px-5 py-5 text-[0.98rem] leading-7 text-slate-800 placeholder:text-slate-400 focus:border-bvm-title/80 focus:outline-none focus:ring-2 focus:ring-bvm-title/20"
                      />
                            ...
                            {PROMPT_CHIPS.map((chip) => (
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  const newAnswer = current ? `${current}, ${chip}` : chip;
                          ...
                          <div className="flex flex-wrap gap-3">
                            {EMOJI_ROWS.map((emoji) => (
                              <button
                                ...
                                onClick={() => {
                                  const current = answers[index];
                                  const newAnswer = current ? `${current} ${emoji}` : emoji;
                                  setAnswer(index, newAnswer);
                                }}
